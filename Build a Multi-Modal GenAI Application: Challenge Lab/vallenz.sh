...
# MASUKAN REGION
read -p "${BLUE_TEXT}${BOLD_TEXT}MASUKAN REGION: ${RESET_FORMAT}" REGION
echo

echo "${BLUE_TEXT}${BOLD_TEXT}Region Saat Ini:${RESET_FORMAT} ${GREEN_TEXT}${REGION}${RESET_FORMAT}"
echo

# Tampilkan Project ID
...
echo "${BLUE_TEXT}${BOLD_TEXT}Generate Gambar... Mohon Bersabar.${RESET_FORMAT}"
/usr/bin/python3 /home/student/GenerateImage.py
echo "${GREEN_TEXT}${BOLD_TEXT}Silahkan cek gambar 'image.jpeg' di directory anda.${RESET_FORMAT}"
...
echo "${GREEN_TEXT}${BOLD_TEXT}Analyzing Gambar dengan Gemini.${RESET_FORMAT}"
